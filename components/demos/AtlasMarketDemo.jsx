...
const CandleChart = () => {
  const candles = [
    ...
    { o: 90, h: 96, l: 84, c: 88, up: false },
    { o: 88, h: 94, l: 82, c: 92, up: true },
    { o: 92, h: 100, l: 88, c: 97, up: true },
  ];
  ...
  return (
    <svg viewBox="0 0 280 110" style={{ width: "100%", height: "110px" }}>
      ...
      {candles.map((c, i) => {
        const x = i * 22 + 4;
        const top = scale(c.h); const bot = scale(c.l);
        const open = scale(c.o); const close = scale(c.c);
        const bodyTop = Math.min(open, close); const bodyH = Math.abs(open - close);
        return (
          <g key={i}>
            <line x1={x + w/2} y1={top} x2={x + w/2} y2={bot} stroke={c.up ? "#00ff88" : "#ff6060"} strokeWidth="1.5" />
            <rect x={x + 2} y={bodyTop} width={w - 4} height={Math.max(bodyH, 2)} fill={c.up ? "rgba(0,255,136,0.7)" : "rgba(255,96,96,0.7)"} rx="1" />
          </g>
        );
      })}
      {/* Gradient area under */}
...
const screens = {
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,rgba(108,99,255,0.15),rgba(0,212,255,0.08))", border: "1px solid rgba(108,99,255,0.3)", borderRadius: 8, padding: "10px 14px", marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#6C63FF", animation: "pulse 1.5s infinite" }} />
            <div>
              <div style={{ fontSize: 10, color: "#6C63FF", fontWeight: 600, letterSpacing: "0.08em" }}>SEÑAL IA DETECTADA</div>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>Patrón alcista · Confianza 94% · Ventana 5 min</div>
            </div>
          </div>
  ...
  signals: {
    ...
    content: () => (
      <div style={{ padding: "16px" }}>
        <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>Señales recientes — Motor IA Atlas</div>
          ...
          {signals.map((s, i) => (
            <div key={i} style={{ background: s.win ? "rgba(0,255,136,0.04)" : "rgba(255,96,96,0.04)", border: `1px solid ${s.win ? "rgba(0,255,136,0.15)" : "rgba(255,96,96,0.15)"}`, borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)" }}>{s.time} · Entrada {s.entry}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: s.win ? "#00ff88" : "#ff6060" }}>{s.result}</div>
                <div style={{ fontSize: 8, color: "rgba(255,255,255,0.25)", marginTop: 2 }}>Conf. {s.conf}%</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 12, background: "rgba(108,99,255,0.06)", border: "1px solid rgba(108,99,255,0.15)", borderRadius: 8, padding: "12px 14px", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
          {[["Win Rate","78.4%","#00ff88"],["Señales hoy","24","#00D4FF"],["ROI mensual","+34.2%","#6C63FF"]].map(([l,v,c]) => (
    ...
    content: () => (
      <div style={{ padding: "16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          {[
            { label: "Balance total", value: "$12,840", color: "#00D4FF" },
            { label: "P&L del día", value: "+$342", color: "#00ff88" },
          ].map((k, i) => (
...
export default function AtlasMarketDemo() {
    ...
    <div style={{ background: "#060610", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(108,99,255,0.2)", fontFamily: "'DM Mono',monospace" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "168px 1fr", minHeight: 380 }}>
        <div style={{ background: "#040410", borderRight: "1px solid rgba(108,99,255,0.1)", padding: "16px 10px" }}>
          <div style={{ fontSize: 8, color: "rgba(108,99,255,0.4)", textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: 12, paddingLeft: 6 }}>Atlas Market</div>
          ...
          <div style={{ marginTop: 16, padding: "0 6px" }}>
            ...
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", marginBottom: 8 }}>Mercados</div>
            {["Cripto","Forex","Índices"].map(m => (
        ...
        <div style={{ overflow: "auto", maxHeight: 420 }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(108,99,255,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ fontSize: 9, color: "#00ff88", background: "rgba(0,255,136,0.08)", padding: "3px 10px", borderRadius: 100, border: "1px solid rgba(0,255,136,0.2)" }}>● Mercado abierto</div>
          </div>
          <Screen />
        </div>
      </div>
