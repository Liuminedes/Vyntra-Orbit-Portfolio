...
function ProjectCard({ project, lang, t, onExpand }) {
  return (
    <div className="group flex flex-col rounded-3xl bg-white/5 border border-white/10 overflow-hidden hover:border-accent/50 transition-colors duration-500 relative">
        ...
        {project.image ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm z-10 gap-4">
              ...
              {project.live && (
                <a href={project.live} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white text-black flex items-center justify-center hover:scale-110 transition-transform" title={t.work.live}>
                  <FiArrowUpRight size={20} />
                </a>
              )}
              {project.github && (
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-[#333] text-white flex items-center justify-center hover:scale-110 transition-transform" title={t.work.github}>
                  <FiGithub size={20} />
                </a>
              )}
            </div>
          ...
          <div className="w-full h-full flex items-center justify-center p-4">
            <MobileProjectCard category={project.category} lang={lang} />
          </div>
        )}
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-4 gap-4">
          ...
          <div className="shrink-0 flex flex-col items-end gap-1.5">
            {project.badge === "in-dev" ? (
              <span className="flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 text-[10px] font-mono uppercase tracking-wider">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-500 animate-pulse"></span>
                {t.work.inDev}
              </span>
            ) : (
              <span className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-500 text-[10px] font-mono uppercase tracking-wider">
        ...
        <div className="mt-auto flex flex-wrap gap-2">
          {project.stack.map((tech, i) => (
            <span key={i} className="px-2.5 py-1 text-[10px] font-mono rounded-full bg-white/5 text-white/50 border border-white/5">
              {tech}
            </span>
          ))}
        </div>
...
export default function Work() {
  ...
  return (
    <section className="min-h-screen py-32 relative z-10">
      ...
      {/* Interactive Demo Modal (real screenshots only) */}
      <AnimatePresence>
        {activeDemo && (
          <motion.div
            initial={{ opacity: 0 }}
