...
export async function POST(req) {
  try {
    const { firstname, lastname, email, phone, service, budget, timeline, message } = await req.json();
    ...
    await Promise.all([
      ...
      transporter.sendMail({
        ...
        to:      destino,
        replyTo: email,
        subject: `[Portfolio] ${firstname} ${lastname}${service ? ` — ${service}` : ""}`,
        headers: { "X-Priority":"1", "X-Mailer":"Vyntra Orbit Portfolio" },
        html: `
          <!DOCTYPE html>
          ...
          <body style="margin:0;padding:0;background:#08070C;font-family:monospace;">
            <div style="max-width:600px;margin:0 auto;padding:32px 24px;">
              ...
              <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
                <tr><td style="padding:8px 0;color:#666;font-size:12px;width:140px;">NOMBRE</td><td style="padding:8px 0;color:#e8e8f0;font-size:14px;font-weight:600;">${firstname} ${lastname}</td></tr>
                ...
                ${phone ? `<tr><td style="padding:8px 0;color:#666;font-size:12px;">TELÉFONO</td><td style="padding:8px 0;color:#e8e8f0;font-size:14px;">${phone}</td></tr>` : ""}
                ${service ? `<tr><td style="padding:8px 0;color:#666;font-size:12px;">SERVICIO</td><td style="padding:8px 0;"><span style="background:rgba(139,92,246,0.15);color:#8B5CF6;padding:3px 10px;border-radius:100px;font-size:12px;border:1px solid rgba(139,92,246,0.3);">${service}</span></td></tr>` : ""}
                ${budget ? `<tr><td style="padding:8px 0;color:#666;font-size:12px;">PRESUPUESTO</td><td style="padding:8px 0;color:#00ff88;font-size:14px;font-weight:600;">${budget}</td></tr>` : ""}
                ${timeline ? `<tr><td style="padding:8px 0;color:#666;font-size:12px;">PLAZO</td><td style="padding:8px 0;color:#00D4FF;font-size:14px;">${timeline}</td></tr>` : ""}
              </table>
              ...
              <div style="text-align:center;padding-top:20px;border-top:1px solid #1a1a30;">
                <a href="mailto:${email}?subject=Re:${service ? ` ${encodeURIComponent(service)}` : ""} — Vyntra Orbit"
      ...
      transporter.sendMail({
        ...
        headers: { "X-Priority":"3", "X-Mailer":"Vyntra Orbit Portfolio" },
        html: `
          <!DOCTYPE html>
          <html><head><meta charset="utf-8"></head>
          <body style="margin:0;padding:0;background:#08070C;font-family:monospace;">
            <div style="max-width:560px;margin:0 auto;padding:40px 24px;">
              ...
              <div style="background:#111120;border:1px solid #1a1a30;border-radius:10px;padding:28px;">
                ...
                <p style="color:#888;font-size:14px;line-height:1.7;margin:0 0 20px;">
                  Recibimos tu mensaje${service ? ` sobre <strong style="color:#8B5CF6;">${service}</strong>` : ""}.<br>
                  Te respondemos en menos de <strong style="color:#e8e8f0;">24 horas</strong> con una propuesta clara.
                </p>
              ...
              <div style="text-align:center;margin-top:24px;">
                <a href="https://vyntra-orbit-portfolio.vercel.app/work" style="color:#8B5CF6;font-size:12px;text-decoration:none;font-family:monospace;">
